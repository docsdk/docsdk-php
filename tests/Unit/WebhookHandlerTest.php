...
class WebhookHandlerTest extends TestCase
{
    ...
    public function testInvalidSignature()
    {
        ...
        $request = new Request('POST', '/webhook', [
            'DocSDK-Signature' => 'invalid'
        ], file_get_contents(__DIR__ . '/requests/webhook_job_finished_payload.json'));

        $this->expectException(SignatureVerificationException::class);

        $this->docSDK->webhookHandler()->constructEventFromRequest($request, 'secret');
    ...
    public function testConstructsWebhookEvent()
    {
        ...
        $request = new Request('POST', '/webhook', [
            'DocSDK-Signature' => '576b653f726c85265a389532988f483b5c7d7d5f40cede5f5ddf9c3f02934f35'
        ], file_get_contents(__DIR__ . '/requests/webhook_job_finished_payload.json'));


        $webhookEvent = $this->docSDK->webhookHandler()->constructEventFromRequest($request, 'secret');
